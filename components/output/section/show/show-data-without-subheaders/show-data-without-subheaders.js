...
import SectionLayoutWithSubheaders
  from "@/components/output/section/section-layout/section-layout-with-subheaders/section-layout-with-subheaders";
...
function ShowDataWithoutSubheaders({
  ...
  const sortDataAndLabelsArrayTogether = () => {
    ...
    if (sort === undefined) {
      const sortedData = colDataArray;
      const sortedLabels = labelDataArray;

      return { sortedData, sortedLabels };
    }

    // Create an array of indices
    const indexedDataArray = colDataArray.map((value, index) => ({ value, label: labelDataArray[index], index }));
